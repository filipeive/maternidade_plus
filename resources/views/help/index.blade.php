...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6" x-data="aiAssistant()">
        ...
        <div class="card-tw overflow-hidden flex flex-col h-[650px]">
            ...
            <div class="card-header-tw bg-gradient-to-r from-brand-600 to-brand-700 text-white p-4">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center text-lg">
                        <i class="fas fa-robot text-gold-300 animate-pulse"></i>
                    </div>
                    <div>
                        <h3 class="text-base font-bold text-white flex items-center gap-2">
                            Assistente IA Maternidade+
                            <span class="bg-gold-400/20 text-gold-300 text-2xs font-semibold px-2 py-0.5 rounded-full border border-gold-400/30">MISAU Moçambique</span>
                        </h3>
                        <p class="text-xs text-white/80">Tire dúvidas sobre utilização do sistema ou diretrizes pré-natais do MISAU</p>
                    </div>
                </div>
                <button @click="clearChat()" class="text-xs text-white/70 hover:text-white flex items-center gap-1">
            ...
            <div class="flex-1 p-4 overflow-y-auto space-y-4 bg-surface-50/50" id="chat-box">
                <template x-for="(msg, index) in messages" :key="index">
                    <div class="flex gap-3" :class="msg.role === 'user' ? 'justify-end' : 'justify-start'">
                        ...
                        <div class="max-w-[85%] rounded-2xl p-3.5 text-xs leading-relaxed shadow-sm"
                ...
                <div x-show="loading" class="flex items-center gap-3">
                    ...
                    <div class="bg-white border border-surface-200 p-3.5 rounded-2xl rounded-tl-none text-xs text-surface-500 flex items-center gap-2">
            ...
            <div class="p-3 border-t border-surface-100 bg-white flex items-center gap-2 overflow-x-auto whitespace-nowrap">
                ...
                <button @click="sendPreset('Como registar uma consulta de puerpério de 7 dias pós-parto?')"
                        class="btn-secondary-tw text-2xs py-1 px-2.5 rounded-full shrink-0">
                    👶 Pós-Parto (Puerpério)
                </button>
            ...
            <div class="p-3 bg-white border-t border-surface-200">
                <form @submit.prevent="sendMessage()" class="flex items-center gap-2">
                    <input type="text"
                           x-model="inputPrompt"
                           placeholder="Digite sua dúvida clínica ou sobre o sistema..."
    ...
    {{-- Right Sidebar: Quick Navigation & Docs (1/3) --}}
    <div class="space-y-6">
        <div class="card-tw">
            ...
            <div class="card-body-tw space-y-3">
                <a href="{{ route('help.manual') }}" class="p-3 rounded-xl border border-surface-200 hover:border-brand-300 hover:bg-brand-50/50 transition-all flex items-center gap-3 group">
                    <div class="w-10 h-10 rounded-lg bg-brand-100 text-brand-700 flex items-center justify-center text-base group-hover:scale-110 transition-transform">
                        <i class="fas fa-book-medical"></i>
                    </div>
                    <div>
                        <h6 class="font-bold text-surface-900 text-xs group-hover:text-brand-700">Manual do Utilizador</h6>
                        <p class="text-2xs text-surface-500">Guia passo a passo de todos os módulos</p>
                    </div>
                </a>

                <a href="{{ route('help.faq') }}" class="p-3 rounded-xl border border-surface-200 hover:border-gold-300 hover:bg-gold-50/50 transition-all flex items-center gap-3 group">
                    <div class="w-10 h-10 rounded-lg bg-gold-100 text-gold-700 flex items-center justify-center text-base group-hover:scale-110 transition-transform">
                        <i class="fas fa-circle-question"></i>
                    </div>
                    <div>
                        <h6 class="font-bold text-surface-900 text-xs group-hover:text-gold-700">Perguntas Frequentes</h6>
                        <p class="text-2xs text-surface-500">Regras de negócio e normas do MISAU</p>
                    </div>
                </a>
            </div>
        </div>
        ...
        <div class="card-tw">
            ...
            <div class="card-body-tw text-xs space-y-2.5">
                <p class="text-surface-600">Para questões operacionais da Unidade Sanitária ou problemas de acesso:</p>
                <div class="p-3 bg-surface-50 rounded-lg space-y-1.5 font-medium">
                    <p class="flex items-center gap-2"><i class="fas fa-envelope text-brand-600"></i> suporte@maternidadeplus.gov.mz</p>
                    <p class="flex items-center gap-2"><i class="fas fa-phone text-brand-600"></i> +258 84 123 4567 / 86 213 4230</p>
                    <p class="flex items-center gap-2"><i class="fas fa-building text-brand-600"></i> MISAU — Direcção de Saúde Zambézia</p>
                </div>
            </div>
        </div>
    </div>
            ...
            async sendMessage() {
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        this.messages.push({ role: 'assistant', content: '❌ ' + (data.message || 'Ocorreu um erro ao consultar a IA.') });
                    }
                } catch (e) {
